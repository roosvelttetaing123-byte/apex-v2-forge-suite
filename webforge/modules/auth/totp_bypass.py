                    ...
                    async with session.get(
                        ...
                        if resp.status in (200, 405):
                            ...
                            if any(kw in body.lower() for kw in
                                ...
                                # Try submitting multiple backup code attempts
                                codes_tried = 0
                                locked = False
                                for code in ["00000001", "00000002", "00000003"]:
                                    async with session.post(
                                        ...
                                        codes_tried += 1
                                        if any(kw in body2.lower() for kw in
            ...
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            # Confirm this is actually an OTP/MFA page before testing
                            body_check = await resp.text(errors="ignore")
                            mfa_kws = ["otp", "one-time", "2fa", "two-factor",
                                       "authentication code", "authenticator", "verification code"]
                            if not any(kw in body_check.lower() for kw in mfa_kws):
                                continue
