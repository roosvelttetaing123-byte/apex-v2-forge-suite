...
CVSS40_MFA_BRUTE = "CVSS:4.0/AV:N/AC:L/AT:N/PR:L/UI:N/VC:H/VI:H/VA:N/SC:N/SI:N/SA:N"
MFA_PATHS = [
...
class MfaBypass(BaseModule):
    ...
    async def _test_response_manipulation(self, mfa_url: str, target: str) -> None:
        ...
        self.new_finding(
            ...
            cvss_v40_vector=CVSS40_MFA_BYPASS,
            target=target,
            url=mfa_url,
        )
    ...
    async def _check_mfa_brute_protection(self, target: str) -> None:
        ...
        # Fingerprint SPA catch-all routing once — avoids false positives on SPAs
        # where every path returns HTTP 200 with the same index.html.
        spa_fp = await self._spa_fingerprint(target)

        for path in MFA_PATHS[:5]:
            url = f"{target}{path}"

            # Validate the endpoint actually exists before brute-forcing.
            # On Vercel/Netlify SPAs, GET requests to non-existent paths return
            # the same index.html — if the GET matches the SPA fingerprint,
            # the endpoint does not exist and testing is pointless.
            await self.rate_limit()
            try:
                async with aiohttp.ClientSession(
                    connector=aiohttp.TCPConnector(ssl=False)
                    ...
                    async with get_session.get(
                        ...
                        get_body = await get_resp.text(errors="ignore")
                        if self._is_spa_body(get_body, spa_fp):
                            continue  # SPA catch-all — endpoint doesn't exist
            except Exception:
                continue
            ...
            if codes_tried >= 5 and not locked:
                ...
                self.new_finding(
                    ...
                    cvss_v40_vector=CVSS40_MFA_BRUTE,
                    target=target,
                    url=url,
                )
                return
