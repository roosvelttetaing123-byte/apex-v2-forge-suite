...
from webforge.core.mode_engine import get_phases, describe_phases, CONFIRM_GATE_MODULES, TOTAL_PHASES
...
async def run_scan(
    ...
    _emit(bus, Event, EventType, "target_discovered", source="webforge",
          target=cfg.target, services=["https"], findings=0)
    ...
    for phase in phases:
        ...
        phase_banner(phase.number, TOTAL_PHASES, phase.name)
    ...
    # Generate reports
    phase_banner(TOTAL_PHASES, TOTAL_PHASES, "Reporting")
...
async def main() -> None:
    ...
    ask_internet_permission("Wappalyzer DB updates, nuclei templates")
