...
class ReportNarrator:
    ...
    async def attack_narrative(
        ...
        if self._brain.available:
            try:
                return await self._ai_attack_narrative(chain_log, memory)
            except Exception as exc:
                log.warning("AI attack_narrative failed, using template: %s", exc)

        return self._template_attack_narrative(chain_log)
    ...
    async def _ai_attack_narrative(
        ...
        prompt = json.dumps({
            ...
            "instructions": (
                ...
                "2. Describe each step: what was tried, what was found, why it matters\n"
                ...
                "6. End with a summary of the full attack chain and its impact\n"
    ...
    def _template_attack_narrative(
        ...
        successes = sum(
            ...
            if any(kw in (s.get("result", "") or "").lower()
                   for kw in ("success", "found", "confirmed", "exploited", "pwned"))
        )
        ...
        sections.append(
            ...
            f"**{successes}** action(s) produced actionable results.\n"
        )
