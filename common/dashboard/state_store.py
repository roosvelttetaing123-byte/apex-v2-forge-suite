...
class StateStore:
    ...
    def _on_scan_start(self, event: Event) -> None:
        with self._lock:
            ...
            self.scan_mode  = event.data.get("mode", self.scan_mode)
            self.engagement = event.data.get("engagement", self.engagement)
            self.tester     = event.data.get("tester", self.tester)
            if event.data.get("target"):
                self.target = event.data["target"]
            if event.data.get("framework"):
                self.framework = event.data["framework"]
            # Auto-register the primary target
            t = self.target
            if t and t not in self.targets:
                self.targets[t] = TargetStatus(target=t)
            ...
            self._add_timeline("scan_start", f"Assessment started → {self.target}", event.source)
        ...
        with self._lock:
            ...
            for phase in self.phases.values():
                if mod_name in phase.modules:
                    phase.findings += 1
                    break
            # Update target finding count — auto-register if needed
            target = d.get("target", self.target)
            if target and target not in self.targets:
                self.targets[target] = TargetStatus(target=target)
