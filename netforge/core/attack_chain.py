...
class AttackChain:
    ...
    def recommend_next(self) -> list[ChainAction]:
        ...
        # --- 4. Brain / Planner Augmentation ---
        try:
            from common.brain.engagement_bus import EngagementBus
            eng_bus = EngagementBus.get_instance()
            if eng_bus and eng_bus.planner:
                brain_actions = eng_bus.planner.get_pending_actions()
                for ba in brain_actions:
                    actions.append(ChainAction(
                        action_id=self._next_id(),
                        phase=ChainPhase.EXECUTION, # generic
                        module=ba.get("action", "unknown"),
                        target=ba.get("target", ""),
                        description=ba.get("reasoning", "Brain suggestion"),
                        priority=3,
                        requires_confirm=True,
                    ))
        except ImportError:
